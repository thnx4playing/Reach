...
export function PrefabNode({ map, name, x = 0, y = 0, scale = 2 }: Props) {
  ...
  // Try to use shared image from context first, fallback to direct loading
  const sharedImage = useMapSkImage();
  const directImage = useImage(def.image);
  const img = sharedImage?.skImage || directImage;
